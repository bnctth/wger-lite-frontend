 ...
 * @constructor
 */
const MutablePaginated = <
  TEditorDto extends Record<string, unknown>,
  TViewDto extends TEditorDto & {
    id: number;
  },
...
>({
  ...
  renderEditor: (
    mutation: Mutation,
    mode: ReducedMode,
    editorItem: TEditorDto,
    setEditorItem: Dispatch<SetStateAction<TEditorDto>>
  ) => ReactNode;
  queryKey: unknown[];
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      switch (mode) {
        case "create":
          return await eitherAsyncToQueryFn(
            apiService.create(endpoint, editorItem)
          )();
        case "edit":
          return await eitherAsyncToQueryFn(
            apiService.update(endpoint, selectedId ?? -1, editorItem)
          )();
        case "delete":
          return await eitherAsyncToQueryFn(
            apiService.delete(endpoint, selectedId ?? -1)
          )();
      }
    },
    onSuccess: async () => {
  ...
  return (
    <PageNameContext.Provider value={name}>
      <div className="h-full w-full flex flex-col items-center gap-10">
        ...
        <Paginated<TViewDto>
          queryFn={(page) => {
            ...
            return eitherAsyncToQueryFn(
              apiService.list(endpoint, offset, limit, parentId, ordering)
            );
          }}
          renderTemplate={(i) =>
            renderTemplate(
